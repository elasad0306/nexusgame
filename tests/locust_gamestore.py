...
class GameStoreUser(HttpUser):
    ...
    def consulter_featured(self):
        ...
        """
        with self.client.get("/games/featured?limit=12", name="/games/featured", catch_response=True) as response:
            if response.status_code != 200:
                return response.failure(f"Code {response.status_code} reçu")

            data = response.json()
            featured = data.get("featured", [])

            # Vérification du contrat : la liste ne doit jamais dépasser 10
            if not isinstance(featured, list):
                response.failure("Format 'featured' invalide")
            elif len(featured) > 10:
                response.failure(f"Limite dépassée : {len(featured)} jeux")
            else:
                response.success()
